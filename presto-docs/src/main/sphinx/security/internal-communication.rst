...
.. note::
    ...
    For ``internal-communication.authentication.krb5.principal`` property a ``_HOST`` placeholder can be used in the value.
    When connecting Presto will substitute this placeholder with the hostname of the node Presto is running on.
    This is useful if each node has its own Kerberos principal.
