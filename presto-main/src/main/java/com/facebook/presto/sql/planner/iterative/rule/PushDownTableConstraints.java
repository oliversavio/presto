        ...
        implements Rule<FilterNode>
{
    ...
    private static final Pattern<FilterNode> PATTERN = filter().with(source().matching(tableScan().capturedAs(TABLE_SCAN)));
    ...
    public Result apply(FilterNode filter, Captures captures, Context context)
    {
        ...
        if (!planChanged(rewrittenPlan, filter, tableScan, context.getLookup(), context.getSession(), context.getSymbolAllocator())) {
            return Result.empty();
        }
    ...
    private boolean planChanged(PlanNode rewrittenPlan, FilterNode oldFilter, TableScanNode oldTableScan, Lookup lookup, Session session, SymbolAllocator symbolAllocator)
    {
        ...
        if (!expressionEquivalence.areExpressionsEquivalent(session, rewrittenFilter.getPredicate(), oldFilter.getPredicate(), symbolAllocator.getTypes())) {
            if (!ImmutableSet.copyOf(extractConjuncts(rewrittenFilter.getPredicate())).equals(ImmutableSet.copyOf(extractConjuncts(oldFilter.getPredicate())))) {
                return true;
            }
        }
