...
{
    // TODO this rule has probably not been reviewed at TD
    ...
    {
        verify(!joinNode.getDistributionType().isPresent());

        JoinDistributionType joinDistributionType = getJoinDistributionType(context.getSession());

        PlanNode rewritten;
        if (joinDistributionType == AUTOMATIC) {
            rewritten = getCostBasedJoin(joinNode, context.getLookup(), context.getSymbolAllocator(), context.getSession(), joinDistributionType);
        }
        else {
            rewritten = getSyntacticOrderJoin(joinNode, context.getLookup(), joinDistributionType);
        }
        return Result.ofPlanNode(rewritten);
    }

    private PlanNode getCostBasedJoin(JoinNode joinNode, Lookup lookup, SymbolAllocator symbolAllocator, Session session, JoinDistributionType joinDistributionType)
    {
        ...
        verify(!possibleJoinNodes.isEmpty());
        return planNodeOrdering.min(possibleJoinNodes).getPlanNode().orElseThrow(IllegalStateException::new);
    }
    ...
    private PlanNode getSyntacticOrderJoin(JoinNode joinNode, Lookup lookup, JoinDistributionType joinDistributionType)
    {
